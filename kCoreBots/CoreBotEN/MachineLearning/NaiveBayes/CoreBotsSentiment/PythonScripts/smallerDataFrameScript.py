...
dfm = pd.read_csv("/root/.encrypted/.pythonSai/kCoreBots/CoreBotEN/MachineLearning/NaiveBayes/CoreBotsSentiment/EntirePoliticalTweetNetwork.csv", sep=",", header=None, usecols=[0,1,2,3,4,5], skiprows=[0], chunksize=2000, names=["tweetid", "userid", "in_reply_to_tweetid", "in_reply_to_userid", "retweet_userid", "retweet_tweetid"])
...
t0 = time.time()
for df_ in dfm:
	df_["retweet_tweetid"] = df_["retweet_tweetid"].astype('Int64')
	df_["in_reply_to_tweetid"] = df_["in_reply_to_tweetid"].astype('Int64')
	t0 = time.time()
	tmp = []
