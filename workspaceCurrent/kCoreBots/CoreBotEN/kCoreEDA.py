...
for df_ in dfn:
	...
	df_lst = df_lst.loc[df_lst["tweet_text"].apply(lambda x: cld3.get_language(str(x)).language=='ru')]
	#for z in eng:
		#df_lst["Yes"] = df_["tweet_text"].apply(lambda x: "true" if str(z).lower() in str(x).lower() and cld3.get_language(str(x)).language=='en' else "false")
	...
	#df_lst[["tweetid", "userid", "tweet_text", "hashtags", "urls", "language"]].to_csv(fsn, mode='a', header=False, index=False)
