...
rba = []
aih = False
#df = pd.read_csv("/root/.encrypted/.pythonSai/ira_tweets_csv_hashed.csv", sep=",", header=None, usecols=[0,1,2,3,6,12,14,15,16,18,19,20,27,28], chunksize=2000, skiprows=1, names=["tweetid", "userid", "user_display_name", "user_screen_name", "user_profile_url", "tweet_text", "tweet_client_name", "in_reply_to_tweetid", "in_reply_to_userid", "is_retweet", "retweet_userid", "retweet_tweetid", "hashtags", "urls"])
...
df_lst = pd.DataFrame(columns=["userid", "is_retweet", "retweet_userid"])
...
#df_lst = df_lst.append(row, ignore_index=True)
			#print(df_lst)
#print(df_lst)
	df_lst += df_.loc[df_.is_retweet.str=="True")]
	df_lst += df_.loc[df_.in_reply_to_userid.str.len()>0)]
	t1 = time.time()
	print(t1-t0)
...
#print(df_lst.loc[df_lst["is_retweet"].isin([True])])
	#df_lst.drop(df_lst.columns[[0, 2]], axis=1)
		...
		#aih = True
	#else:
	#df_lst[["tweetid", "userid", "user_display_name", "user_screen_name", "user_profile_url", "tweet_text", "tweet_client_name", "in_reply_to_tweetid", "in_reply_to_userid", "retweet_userid", "retweet_tweetid", "hashtags", "urls"]].to_csv('my_parsed4.csv', mode='a', header=False, index=False)
...
#kcg = k_core(G,k=118)
#kci = list(kcg.nodes)
#print(number_strongly_connected_components(kcg))
#dfb = pd.DataFrame(data={"col1": kci})
#dfb.to_csv("kCoreBotsList.csv", sep=',',index=False)
...
for df_ in df:
	t0 = time.time()
	df_lst = df_.loc[df_["userid"].map(lambda x: x in kci)]
	t1 = time.time()
	print(t1-t0)
	
	df_lst[["tweetid", "userid", "user_display_name", "user_screen_name", "user_profile_url", "tweet_text", "tweet_client_name", "in_reply_to_tweetid", "in_reply_to_userid", "retweet_userid", "retweet_tweetid", "hashtags", "urls"]].to_csv('my_parsed5.csv', mode='a', header=False, index=False)
	t2 = time.time()
	print(t2-t0)

for df_ in df:
	t0 = time.time()
	df_lst = df_.loc[df_["retweet_userid", ].map(lambda x: x in kci)]
	t1 = time.time()
	print(t1-t0)
...
import time
#kcg = k_core(G,k=1)
#dkc=[(1,len(kcg.nodes()),number_strongly_connected_components(kcg))]
#print(dkc)
...
for i in range(0,10):
	...
	if(i==0):
		...
		t1 = time.time()
		print(t1-t0)
		dkc=[(i,len(G.nodes()),number_strongly_connected_components(G))]
		#dkc.append((i,len(G.nodes()),((len( G.edges()))/ncr(len(G.nodes()), 2))))
		t2 = time.time()
		print(t2-t0)
		dfn = pd.DataFrame(dkc, columns=["kVal","NoOfCoreBots","NoOfCoreCC"])
		dfn.to_csv('kCoreStatGraphSCC1.csv', mode='a', header=True)
	else:
		t0 = time.time()
		print("hey")
		kcg = k_core(G,k=i)
		t1 = time.time()
		print(t1-t0)
		dkc=[(i,len(kcg.nodes()),number_strongly_connected_components(kcg))]
		#dkc.append((i,len(kcg.nodes()),((len(kcg.edges()))/ncr(len(kcg.nodes()), 2))))
		t2 = time.time()
		print(t2-t0)
		dfn = pd.DataFrame(dkc, columns=["kVal","NoOfCoreBots","NoOfCoreCC"])
		dfn.to_csv('kCoreStatGraphSCC1.csv', mode='a', header=False)


'''
for i in range(0,120):
	if(i==0):
		dkc.append((i,len(G.nodes()),len(G.edges())))
		dfn = pd.DataFrame(dkc, columns=["kVal","NoOfCoreBots","NoOfCoreEdges"])
		dfn.to_csv('kCoreStats.csv', mode='a', header=True)
	else:
		kcg = k_core(G,k=i)
		dkc=[(i,len(kcg.nodes()),len(kcg.edges()))]
		dfn = pd.DataFrame(dkc, columns=["kVal","NoOfCoreBots","NoOfCoreEdges"])
		dfn.to_csv('kCoreStats.csv', mode='a', header=False)
'''
	#print(t1-t0)
